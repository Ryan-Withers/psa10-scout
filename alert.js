...
function selectAlerts(rows) {
  ...
  const actNames = new Set(act.map((r) => String(r.player || '').toLowerCase()));
  const also = bestPerPlayer(
    rows.filter((r) => !r.verdict?.shout && r.verdict?.call === 'BUY'
      && !actNames.has(String(r.player || '').toLowerCase()))
  ...
  <table cellpadding="0" cellspacing="0" border="0" width="100%"><tr>
    ...
    <td valign="top">
      ...
      <div style="font-size:13px;color:#3a3a3a;margin-bottom:3px">
        ...
        ${r.compAud ? `<span style="color:#0a7d4a;font-weight:700;margin-left:8px">${pct(r.edge)} under</span>
...
function cardText(r) {
  const v = r.verdict || {};
  return `${v.call}${(v.tags || []).length ? '  [' + v.tags.join(' ') + ']' : ''}\n${v.hook}\n${r.player} - ${cardName(r)}\n${money(r.landedAud)} landed${r.compAud ? `, ${pct(r.edge)} under the ${money(r.compAud)} these go for` : ''}\n${r.url}`;
}
...
function subject({ act, also }) {
  if (!act.length && !also.length) return 'Nothing worth flagging';
  const targets = act.filter((r) => (r.verdict?.tags || []).includes('TARGET'));
  if (targets.length) {
    const t = targets[0];
    return `${t.player} at ${pct(t.edge)} under, one of yours`;
  }
  if (act.length === 1) {
    const r = act[0];
    return `${r.player}, ${pct(r.edge)} under - ${r.verdict.call.toLowerCase()}`;
  }
  if (act.length) {
    const best = act[0];
    return `${act.length} to look at, best is ${best.player} at ${pct(best.edge)} under`;
  }
  return `${also.length} solid buys, nothing urgent`;
}
...
function buildAlert(selection, { placeholder = false, trimmed = 0 } = {}) {
  ...
  const heading = (t) => `<div style="font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:#8a8a8a;font-weight:700;margin:18px 0 10px">${esc(t)}</div>`;
...
<div style="font-size:20px;font-weight:700;color:#1a1a1a;margin-bottom:2px">${act.length ? `${act.length} worth acting on` : 'Nothing urgent'}</div>
